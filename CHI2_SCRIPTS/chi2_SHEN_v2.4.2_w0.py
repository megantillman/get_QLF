...
norm_locals = np.linspace(6, 10, reso)
combos = np.array(list(itertools.product(xsigpost, norm_locals)))



filename = "output/chi2_SHEN_r"+str(reso)+"_v2.4.2_w0_s2.h5py"
...
dset = f.create_dataset('norm_local', data = norm_locals)

f.close()

def chi2(a, z, qlf):
    qlf.get_Mbh(logMstar0, slopes, norms, approx_local=True, norm_local = a[1])
