...
filename = "output/check_emcee_chain-data_2Param_z="+str(redshift)+".h5py"

bin_num = 100
sig_lnMstar = 0.7
lnxsig1 = 3.5
ndim, nwalkers, steps, runs = 2, 400, 250000, 40
burn_in = 500
threads = 12
...
def lnprior(p):
    if 0.0 < p[0] <= 3.0 and 0.5 < p[1] <= 10.0:
        return 0.0
    return -np.inf
...
def model_func(x, p):
    qlf.get_SMBM(p[0])
    qlf.get_dNdlnL(lnxsigs = [lnxsig1,p[1]])
...
sampler = emcee.EnsembleSampler(nwalkers, ndim, lnprob, threads = threads, args=(np.array(x), np.array(y), np.array(yerr)))
    
pos = [np.array([np.random.uniform(low=0, high=3), np.random.uniform(low=0, high=10)]) for i in range(nwalkers)]
pos = np.array(pos)
pos[(pos <= 0.0)] = 0.01
...
f.attrs.modify('sigmalnM*', sig_lnMstar)
f.attrs.modify('bin-num', bin_num)
f.attrs.modify('siglnX1', lnxsig1)
f.attrs.modify('model-info', 'QLF fit uses the universal mdot distribution model as well as the luminosity dependant obscuration factor.')
...
dset = f.create_dataset('pre-burnin-position:siglnX2', data = sampler.chain[:,0,1])
f.close()
