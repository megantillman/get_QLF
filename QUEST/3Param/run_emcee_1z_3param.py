...
bin_num = 100
sig_lnMstar = 0.7
ndim, nwalkers, steps, runs = 3, 100, 100, 3
burn_in = 100
...
def run_emcee(z, filename):
    ...
    def lnprior(p):
        if 0.0 < p[0] <= 3.0 and 0.0 < p[1] <= 10.0 and 0.0 < p[2] <= 15.0:
            return 0.0
        return -np.inf
    ...
    def model_func(x, p):
        qlf.get_SMBM(p[0])
        qlf.get_dNdlnL(lnxsigs = [p[2],p[1]])
    ...
    dset = f.create_dataset('pre-burnin-position:siglnX1', data = sampler.chain[:,0,2])
    f.close()
