...
filename = "/mnt/home/mtillman/ceph/QLF_proj/output/chi2_L_z"+str(z)+".h5py"

r1, r2, r3, r4, r5, r6 = 1, 1, 100, 1, 1, 100

logMstar0 = [3.0]
xsigpre = [4.0]
xsigpost = np.linspace(0.01,5.0,r3)
slopes = [1.0]
norms = [0.0]
local_norms = np.linspace(-4.0, -1.0, r6)
...
def chi2(a):
    ...
    dNdlogL = get_QLF(lums, logMstar0 = a[0], slope_low = a[3], norm_from_local = a[4], norm_local = 11+a[5], presig = a[2], postsig = a[2])
...
def SHEN_fit(z, lums): ###best fit data from Shen+2020
    #### insert lums as log10 of the bolometric lum value
...
print('Inserting phantom point.')

xobscut = min(np.asarray(xtot))
xphan = np.linspace(40, 44, 9)
# xphan = np.linspace(40, 44, 17)
if round(xobscut) < xobscut:
    xphan = xphan[xphan <= round(xobscut)]
else:
    xphan = xphan[xphan < round(xobscut)]
yphan, yphanerr, _, _, _ = SHEN_fit(z, np.log10(10**xphan/3.8e33))
# print('Phantom point errors:',yphanerr)
# yphanerr[yphanerr > 0.2] = 0.2
# print('Phantom point errors after capping at 0.2 dex:', yphanerr)

ya = np.array(ya)
ya = np.append(ya, yphan)
yaerr = np.array(yaerr)
yaerr = np.append(yaerr, yphanerr)
lums = np.log10(10**np.asarray(xtot)/3.8e33)
lums = np.append(lums, np.log10(10**xphan/3.8e33))
